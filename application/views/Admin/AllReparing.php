                        ...
                        <tbody>
                            <?php if (!empty($piercings)) {
                                ...
                                foreach ($piercings as $piercing) {
                                    $statuses = ["Pending", "Repair", "Cleaning", "Received", "Inspection", "Karigar", "Working", "Delivery"];
                                    $currentStatus = ucfirst($piercing->status);
                                    ...
                                    <tr>
                                        ...
                                        <td><?= ucfirst($piercing->jweleryType); ?></td>
                                        <td><?= ucfirst($piercing->repairNeed); ?></td>
                                        <td style="max-width: 200px; overflow-wrap: break-word;">
                                            <?= ucfirst($piercing->description); ?>
                                        </td>
                                        <td><?= date('d-m-Y', strtotime($piercing->created_at)); ?></td>

                                        <td>
                                            <select class="form-select status-dropdown" data-id="<?= $piercing->id ?>">
                                                <?php
                                                foreach ($statuses as $index => $status) {
                                                    $selected = ($currentStatus === $status) ? 'selected' : '';
                                                    $disabled = ($index < $currentIndex) ? 'disabled' : ''; // Disable all before current
            ...
            dropdown.addEventListener('change', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        fetch("<?= base_url('AdminController/updateReparingStatus') ?>", {
                            method: "POST",
                            headers: {
                        ...
                        location.reload(); // revert to previous
                    }
                });
            });
        });
    ...
    <script>
        ...
        document.querySelector(".btn-yes").addEventListener("click", () => {
            if (selectedCustomerId) {
                fetch("<?= site_url('AdminController/deletReparingAppointmentById') ?>", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded"
                    },
