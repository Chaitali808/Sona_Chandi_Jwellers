...
<body>
            ...
            <div class="col-lg-9 col-xl-10 col-12 offset-xl-2 offset-lg-3 text-white ">
                ...
                <div class="container  m-2  d-flex justify-content-start  align-items-center p-2 pt-4">
                    <form class="d-flex p-2  w-50" role="search">
                        <input type="text" class="form-control rounded-start-5 p-2 rounded-end-0 border-end-0 "
                            ...
                            <?php if (!empty($customers)) {
                                ...
                                foreach ($customers as $customer) { ?>
                                    <tr>
                                        ...
                                        <td><?= $customer->dob; ?></td>
                                        <td><?= $customer->in_time; ?></td>
                                        <td><?= $customer->out_time; ?></td>
                                        <td><?= $customer->feedback_date; ?></td>
                                        <td>
                                            <i class="fa-solid fa-trash del-btn" data-id="<?= $customer->id ?>"
    ...
    <div class="popup-overlay popup-overlay-del" style="display:none;">
        <div class="popup-box text-center p-4 bg-white rounded shadow">
            <div class="popup-icon mb-2"><i class="fas fa-exclamation fa-2x " style="c<script>
  const rowsPerPage = 10;
  const table = document.getElementById(" ordersTable"); const tbody=table.querySelector("tbody"); const
                    rows=Array.from(tbody.querySelectorAll("tr")); const
                    pagination=document.getElementById("pagination"); let currentPage=1; function showPage(page) { const
                    start=(page - 1) * rowsPerPage; const end=start + rowsPerPage; rows.forEach((row, index)=> {
                    row.style.display = index >= start && index < end ? "" : "none" ; });
                        updatePaginationButtons(rows.length, page); } function updatePaginationButtons(totalRows,
                        activePage) { const totalPages=Math.ceil(totalRows / rowsPerPage); pagination.innerHTML="" ; //
                        Previous button (icon only) const prev=document.createElement("li"); prev.className="page-item"
                        + (activePage===1 ? " disabled" : "" ); const prevLink=document.createElement("a");
                        prevLink.className="page-link" ; prevLink.href="#" ; prevLink.innerHTML="&laquo;" ;
                        prevLink.onclick=e=> {
                        e.preventDefault();
                        if (currentPage > 1) {
                        currentPage--;
                        ...
                        // Page number buttons
                        for (let i = 1; i <= totalPages; i++) { const li=document.createElement("li");
                            li.className="page-item" + (i===activePage ? " active" : "" ); const
                            link=document.createElement("a"); link.className="page-link" ; link.href="#" ;
                            link.textContent=i; link.onclick=e=> {
                            e.preventDefault();
                            currentPage = i;
                            showPage(currentPage);
                            };
                            ...
                            if (currentPage < totalPages) { currentPage++; showPage(currentPage); } };
                                next.appendChild(nextLink); pagination.appendChild(next); } // Initial page load
                                showPage(currentPage); </script>
                                olor:#915cd1;"></i></div>
            <div class="popup-message fw-bold">Are you sure?</div>
            <div class="popup-sub text-muted mb-3">Do you want to delete the Feedback?</div>
    ...
    <script>
        ...
        document.querySelector(".btn-yes").addEventListener("click", () => {
            if (selectedCustomerId) {
                fetch("<?= site_url('AdminController/deletFeedbackById') ?>", {
                    method: "POST",
                    headers: {
